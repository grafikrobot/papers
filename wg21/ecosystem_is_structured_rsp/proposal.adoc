...
_structured options_ schema (<<strctparam-schema>>).
...
structured options (<<strctparam>>) to process.
...
==== Annex B (informative) Structured Options _JSON Schema_ [.right]#[<<strctparam-json-schema>>]#
...
(<<strctparam-schema>>) in terms of a _JSON Schema_. A _JSON Schema_ refers to the
