...
==== Structured [.chg]#Parameters# [.right]#[<<strctparam>>]#
...
The pathname of a file to read the _structured [.chg]#parameters#_ from.
...
_structured [.chg]#parameters#_ schema (<<strctparam-schema>>).
...
`__std-strctparam-input__` [.chg]#parameter# of the list of
[.chg]#parameters# given to the application. Given either directly as part of
the application command line or as part of the arguments field
(<<strctparam-field-args>>).

[.chg]
An application shall _process_ an argument or option in the file by applying the
semantics to change the state of the application that is relevant to the
argument or option at the point that the application sees the argument or
option.

[#strctparam-schema,reftext=strctparam.schema]
===== Schema [.right]#[<<strctparam-schema>>]#

A _structured [.chg]#parameters#_ JSON text file shall contain one
_structured [.chg]#parameters#_ JSON object (<<strctparam-schema-param>>).

[#strctparam-schema-param,reftext=strctparam.schema.param]
===== Structured [.chg]#Parameters# Object [.right]#[<<strctparam-schema-param>>]#

The _structured [.chg]#parameters# object_ is the root JSON object of the
structured [.chg]#parameters# JSON text.

A _structured [.chg]#parameters# object_ can have the following fields.

[.chg]
A _structured [.chg]#parameters# object_ shall have only one of `arguments` and
`options` fields.
...
_Description_:
  A _structured [.chg]#parameters# object_ can contain this field.
  If a _structured [.chg]#parameters# object_ does not contain this field the
...
_Description_:
  The version field indicates the version of the _structured [.chg]#parameters#_
  represented in the contents of the JSON text.
  [.chg]#If a _structured parameters object_ does not contain this field the
...
===== Names [.right]#[<<strctparam-strctparam-names>>]#
...
[#strctparam-flag-opt,reftext=strctparam.schema.flag]
===== Flag Option [.right]#[<<strctparam-flag-opt>>]#
...
[#strctparam-struct-opt,reftext=strctparam.schema.struct]
===== Structured Option [.right]#[<<strctparam-struct-opt>>]#
...
[#strctparam-opt-opt,reftext=strctparam.schema.opt.opt]
===== Option `std.[.chg]#param#` [.right]#[<<strctparam-opt-opt>>]#

The `std.[.chg]#param#` option defines an option to refer to additional
_structured [.chg]#parameters#_ (<<strctparam>>) to process.

An application shall implement this option.

The `std.[.chg]#param#` option shall have the following fields.

_Name_: `name` +
_Type_: `string` +
_Value_: `std.[.chg]#param#` +
_Description_: The name of the option.

_Name_: `files` +
_Type_: `string` or `array` +
_Value_:
  (for `string`) A pathname to a file containing _structured [.chg]#parameters#_
  (<<strctparam>>). +
_Value_:
  (for `array`) A list of pathname `string` items to files containing
  _structured [.chg]#parameters#_ (<<strctparam>>) +
_Description_:
  One or more references to files that include additional _structured
  [.chg]#parameters#_ (<<strctparam>>) that are processed as if they occur at
  the location of the `std.[.chg]#param#` option.
...
==== Annex B (informative) Structured [.chg]#Parameters# _JSON Schema_ [.right]#[<<strctparam-json-schema>>]#

[#strctparam-json-schema-general,reftext=strctparamjschm.general]
===== General [.right]#[<<strctparam-json-schema-general>>]#

This Annex defines the structured [.chg]#parameters# capability schema
