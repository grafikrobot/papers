...
Change wording to match text in draft IS. Use Unicode for literals as needed.
Change the version numbers to follow the Semantic Versioning, and JSON, specs
...
_patch-number_: ::
  _decimal_
_decimal_: ::
  one of: U+0031 .. u+0039 DIGIT ONE .. NINE +
  zero or more of: U+0030 .. u+0039 DIGIT ZERO .. NINE

A _decimal_ is a sequence of decimal digits (U+0030 .. u+0039 DIGIT ZERO .. NINE)
with no superfluous leading zeros (U+0030 DIGIT ZERO).

A _decimal_ represents a non-negative base 10 integer.

A version number is composed of 1, 2, or 3 decimal numbers (_decimal_)
separated by a U+002E FULL STOP.
...
Version numbers define a total ordering where version number _a_ with decimal
parts _i_, _j_, _k_ is ordered before version number _b_ with decimal parts
_p_, _q_, _r_ when: _i_ is less than _p_, or _i_ is equal to _p_ and
_j_ is less than _q_, or _i_ is equal to _p_ and _j_ is equal to _q_
and _k_ is less than _r_.

Otherwise version number _a_ is ordered before version number _b_ when:
_i_ is greater than _p_, or _i_ is equal to _p_ and _j_ is greater than _q_, or
_i_ is equal to _p_ and _j_ is equal to _q_ and _k_ is greater than _r_.
