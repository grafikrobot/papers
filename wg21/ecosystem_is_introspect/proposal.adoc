...
introspection trivial and declaration straightforward.
...
`--std-info` [.right]#[<<intspct-opt-info>>]#::
...
`--std-info-out=_file_` [.right]#[<<intspct-opt-out>>]#::
...
`--std-info=_declaration_` [.right]#[<<intspct-opt-decl>>]#::
...
capability [.right]#[<<intspct-fld-cap>>]# ::
...
_Description_:
  ...
  unique withing the _introspection object_.
...
A single version shall be equivalent to the inclusive version range spanning
solely that single version number.
...
An application shall output an an introspection schema (<<intspct-schema>>)
that contains one capability field for each capability that the application
supports when given the `--std-info` option (<<intspct-opt-info>>).
