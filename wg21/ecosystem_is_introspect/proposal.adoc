...
Addition of scope, levels, use cases, and wording.
...
tools must support or adhere to. Ultimately it only wants to make it possible
to answer two questions:

* What does the tool support and adhere to?
* Does the tool support and adhere to X?
...
— An application shall support the _minimum level_ of introspection
(<<intspct-min>>).
...
+[+ _Note 1_: From ISO/IEC 9945:2009. -- _end note_ +]+
...
capabilities of this standard an application implements. An application can
implement either the minimum or full level functionality.
...
An application that implements the minimum level functionality indicates it by
...
An application can implement the full level functionality as defined in this
section. An application that reports implementing the full level shall implement
all of the functionality in this section.

An application that implements the full level functionality indicates it by
specifying a version range (<<intspct-vers-single>>) as the value of the
`std:info` capability (<<intspct-cap>>).

+[+ _Example 1_: +
`{ "std:info": "[1.0.0,1.0.0]" }` +
— _end example_ +]+
...
_capability-identifier_: ::
  _name_ _scope-designator_ _name_ _sub-capability-identifier_
_sub-capability-identifier_: ::
  _scope-designator_ _name_ _sub-capability-identifier_
_name_: one or more of ::
...
The _name_ part `std` in a _capability-identifier_ is reserved for capabilities
...
A version shall be either a single version (<<intspct-vers-single>>) or version
range (<<intspct-vers-range>>).
...
[horizontal]
_version-number_: ::
  _major-number_ `.` _minor-number_ `.` _patch-number_
_major-number_: ::
  _digits_
_minor-number_: ::
  _digits_
...
_version-range_: ::
  ...
  _version-min-number_ `,` _version-max-number_
...
or two version numbers separated by a command (`,`) and bracketed.
...
A version range _a_ that is `[_i_,_j_)` makes _j_ and exclusive version number.
...
Otherwise the resulting range when _a_ is compared to _b_ is the exclusive version numbers "lesser of _i_ and _m_" and "lesser of _j_ and _n_".
