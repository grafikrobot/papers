...
<<P1112>> proposes a mechanism to have "smart" algorithmic layout control.
It proposes to add a `[[layout(?)]]` attribute to the class to select from
an existing set of algorithmic layouts like: `smallest`, `declorder`,
`cacheline`, and so on. A key problem with an algorithmic approach is the
increased risk of ABI violations as pointed out in <<P1112>>. Dealing with
the {CPP} ABI is difficult enough as it is. We would like to avoid adding
to the uncertainly and complexity of the {CPP} ABI.
